...
Nothing above is reported to the page — not which values were picked, not that
a value is missing, not that playback failed. All of it goes to the server log
at startup and to `console.error` in the browser.

That is a reversal of what this demo did earlier in its life, and it is the
point rather than an oversight. A demo whose lesson is "here is how you
configure it" should show its configuration. A demo whose lesson is "here is
what it looks like live" must not, because a real site doesn't — and a visitor
who sees `DEMO_FIXED_AVATAR_ID` on a product page has learned the wrong thing
about what shipping this looks like. The honesty lives here and in the log,
...
// there is nothing to converse with, so nothing renders.
