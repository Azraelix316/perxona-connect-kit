...
Optional: `PORT`; `DEMO_DEFAULT_*` (avatar/scene/voice/motion defaults for the Starter demo) and `DEMO_FIXED_*` (fixed
avatar/scene/voice target, hides the catalog pickers) — see the env var table in `README.md` for the full list; and
`LLM_API_KEY` (+ `LLM_PROVIDER`, `LLM_BASE_URL`, `LLM_MODEL`) to enable the chat panel. Keep secrets in `.env` and never
