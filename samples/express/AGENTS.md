...
- `demos/embed/` — a product page with an avatar answering questions on it. No catalog and no pickers, and nothing anywhere
  ...
  (`fixedTarget`, `chatbotId`), and when either is missing the widget is not rendered and the reason goes to
  `console.error`. A live site does not explain its own setup to visitors, so this one doesn't either. The first message
  you send is what unlocks audio — no separate "enable audio" control, because a real site has none.
